...
namespace MyRealEstate
{
    public partial class AdminHomePage : Form
    {
            ...
            else
            {
                ...
                try
                {
                    using (Real_Estate_App.Model.RealEstateEDM r = new Real_Estate_App.Model.RealEstateEDM("Admin"))
                    {
                        ...
                        r.Add_Property_Photo((int)d.FirstOrDefault().PropertyID,ImageToByteArray(circularPictureBox3.Image));
                    }
                    MessageBox.Show("Property was added successfully!");
                    clear();
                }
                catch (Exception ex)
                {
        ...
        public void PropertyClicked(object sender, EventArgs e)
        {
            using (RealEstateEDM rdm = new RealEstateEDM("Admin"))
            {
                var obj = (DisplayProperty)sender;
                var SearchResult = rdm.Search_Property_By_ID(int.Parse(obj.ID)).FirstOrDefault();
                DetailPage d = new DetailPage(SearchResult, rdm.Search_Property_Photo_By_ID(SearchResult.ID).ToList<Search_Property_Photo_By_ID_Result>(), "Admin", "", "");
                d.ShowDialog();
                d.UserType = "Admin";
            }
        }
        public void Display(List<Get_All_Properties_Result> properties)
        {
        ...
        private void ShowAllPropPanel_Paint(object sender, PaintEventArgs e)
        {

            using (RealEstateEDM r = new RealEstateEDM("Admin"))
            {
                Properties = r.Get_All_Properties().ToList<Get_All_Properties_Result>();
            }
            PropRefresh_lbl_Click(sender, e);
        }
        ...
        private void SearchtextBox_TextChanged(object sender, EventArgs e)
        {
            if (Properties != null)
            {
                List<Get_All_Properties_Result> properties = Properties.Where(prop => prop.ID.ToString().Contains(SearchtextBox.Text.ToString()) || prop.Address.Contains(SearchtextBox.Text) || prop.Price.ToString().Contains(SearchtextBox.Text)).ToList();
                Display(properties);
            }
        }
    }
